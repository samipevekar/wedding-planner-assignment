...
import { saveGuestsToStorage } from '../services/storage';

const GuestList = ({ route }) => {
  const { guests: initialGuests, onDeleteGuest } = route.params;
  ...
  // keep sync with parent (if guests updated in HomeScreen)
  useEffect(() => {
    if (route.params?.guests) {
      setGuestsState(route.params.guests);
    }
  }, [route.params?.guests]);

  // In GuestList.js - Update the handleDelete function
const handleDelete = async (guestId) => {
  // 1. Create the updated state first
  const updatedGuests = guestsState.filter(g => g.id !== guestId);
  
  // 2. Update local state for instant UI response
  setGuestsState(updatedGuests);
  
  // 3. Save the UPDATED guests to storage
  await saveGuestsToStorage(updatedGuests);
  
  // 4. Update parent state (HomeScreen)
  if (onDeleteGuest) {
  ...
  const filteredGuests = useMemo(() => {
    return guestsState.filter(guest => {
      const matchesSearch = guest.name.toLowerCase().includes(searchQuery.toLowerCase());
      const matchesFilter = filterStatus === 'All' || guest.rsvpStatus === filterStatus;
      return matchesSearch && matchesFilter;
    });
  }, [guestsState, searchQuery, filterStatus]);

  // Calculate statistics
  const stats = useMemo(() => {
    const total = guestsState.length;
    const confirmed = guestsState.filter(g => g.rsvpStatus === 'Yes').length;
    const maybe = guestsState.filter(g => g.rsvpStatus === 'Maybe').length;
    const declined = guestsState.filter(g => g.rsvpStatus === 'No').length;
  ...
  const EmptyState = () => (
    <View style={styles.emptyStateContainer}>
      ...
      <Text style={styles.emptyStateTitle}>
        {guestsState.length === 0 ? 'No Guests Yet' : 'No Results Found'}
      </Text>
    ...
    return (
      ...
      >
        ...
        <View style={[
          styles.filterCount,
          { 
            backgroundColor: isSelected ? 'rgba(255,255,255,0.2)' : config.color,
          }
        ]}>
          <Text style={[
  ...
  const ListHeader = () => (
    <View style={styles.listHeader}>
      <Text style={styles.listHeaderText}>
        Showing {filteredGuests.length} of {guestsState.length} guests
      </Text>
      {searchQuery && (
