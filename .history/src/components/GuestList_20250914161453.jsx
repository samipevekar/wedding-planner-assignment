...
const GuestList = ({ route }) => {
  ...
  // ✅ Fixed delete handler - immediate local update, then parent update
  const handleDelete = async (guestId) => {
    // 1. Immediately update local state for instant UI response
    // setGuestsState(prev => prev.filter(g => g.id !== guestId));
    // await saveGuestsToStorage(guestsState)
    
    // 2. Update parent state (HomeScreen) - no delay needed
    if (onDeleteGuest) {
