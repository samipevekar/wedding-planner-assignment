...
import { loadGuestsFromStorage, saveGuestsToStorage } from '../services/storage';

const { width, height } = Dimensions.get('window');

const GuestList = ({ route, navigation }) => {
  const { onDeleteGuest } = route.params;
  ...
  // Load guests when screen focuses
  useFocusEffect(
    useCallback(() => {
      loadGuests();
    }, [])
  ...
  const handleDelete = async (guestId) => {
    if (onDeleteGuest) {
      ...
      // Reload the guests after a short delay to ensure storage is updated
      setTimeout(() => {
        loadGuests();
      }, 300);
    }
