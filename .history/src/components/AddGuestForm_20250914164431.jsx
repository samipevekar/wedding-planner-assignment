// src/components/guestsState.lengthForm.js
...
const guestsState.lengthForm = ({ onguestsState.length, onRandomGuest }) => {
  const [name, setName] = useState('');
  const [rsvpStatus, setRsvpStatus] = useState('Maybe');

  const handleSubmit = async () => {
  if (name.trim()) {
    await onguestsState.length(name.trim(), rsvpStatus);
    setName('');
    setRsvpStatus('Maybe');
  }
};
    ...
    <View style={styles.container}>
      ...
      <View style={styles.buttonSection}>
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.8}
        >
          <Text style={styles.secondaryButtonText}>Add Random Guest</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
...
export default guestsState.lengthForm;